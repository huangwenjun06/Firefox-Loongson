...
class CodeGeneratorMIPS64 : public CodeGeneratorShared
{
    friend class MoveResolverMIPS64;

    CodeGeneratorMIPS64* thisFromCtor() {
        return this;
    }
  ...
  protected:
    ...
    void bailoutCmp32(Assembler::Condition c, T1 lhs, T2 rhs, LSnapshot* snapshot) {
        Label skip;
        masm.branch32(Assembler::InvertCondition(c), lhs, rhs, &skip, ShortJump);
        bailout(snapshot);
        masm.bind(&skip);
    }
    template<typename T>
    ...
    void bailoutCmpPtr(Assembler::Condition c, T1 lhs, T2 rhs, LSnapshot* snapshot) {
        Label skip;
        masm.branchPtr(Assembler::InvertCondition(c), lhs, rhs, &skip, ShortJump);
        bailout(snapshot);
        masm.bind(&skip);
    }
...
class OutOfLineBailout : public OutOfLineCodeBase<CodeGeneratorMIPS64>
{
    LSnapshot* snapshot_;
    uint32_t frameSize_;
